...
const NotificationCard = ({data,index,notificationState}) =>{
    ...
    let [isRepying , setReplying]  = useState(false);

    let {reply ,type,replied_on_comment,createdAt,comment,user,user:{personal_info:{ profile_img , fullname , username}},blog:{_id,blog_id,title}, _id :notification_id,seen} = data;
    ...
    return (
        ...
        <div className={"p-6 border-b border-grey border-l-black " + (!seen ? "border-l-2" : '')}>
             ...
             <div className="flex gap-5 mb-3 ">
                    <img src={profile_img} alt="" className="w-14 h-14 rounded-full flex-none" />
                    <div className="w-full">

                        <h1 className="font-medium text-xl text-dark-grey">

                             <span className="hidden lg:inline-block capitalize">{fullname}</span>
                             <Link to={`/user/${username}`} className="mx-1 text-black">@{username}</Link>
...

                        {
                            ...
                            </div>  : <Link className="font-medium text-dark-grey hover:underline line-clamp-1" to={`/blog/${blog_id}`}>{`"${title}"`}</Link>
                        }
...

             {
                ...
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md ">
                    ...
                    <div className="flex gap-3 mb-3">
                        ...
                        <div>
                            <h1 className="font-medium text-xl text-dark-grey"><Link className="mx-1 text-black " to={`/user/${author_username}`}>{author_username}</Link>

                            <span className="font-normal">replied to</span>

                            <Link className="mx-1 text-black " to={`/user/${username}`}>
                                  {username}
                            </Link>
                    ...
                    <button className=" hover:text-black ml-14 mt-2" onClick={(e)=> handledelete(comment._id , "reply" , e.target) }>Delete</button>
